...
{
	if (pUI) pUI->chat()->addInfoMessage("CNetGame processs");
...
void CNetGame::ProcessLoadingScreen()
{
	...
	CCameras::SetPosition(1093.0, -2036.0, 90.0, 0.0, 0.0, 0.0);
	CCameras::LookAtPoint(384.0, -1557.0, 20.0, 2);
...
void CNetGame::Packet_MarkerSync(Packet *pkt)
{
    ...
    if (iNumberOfPlayers) {
        for (int i = 0; i < iNumberOfPlayers; i++) {
            ...
            if (playerId < MAX_PLAYERS && GetPlayerPool()->GetSlotState(playerId)) {
