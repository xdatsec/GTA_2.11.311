...
public:
    ...
    [[nodiscard]] const CVector GetPosn() const { return UncompressLargeVector(m_vecPos); } // 0x4549A0
    [[nodiscard]] CVector2D GetPosn2D() const { return static_cast<const CVector &>(GetPosn()); };    // NOTSA
    ...
    void ExtractAmmoFromPickup(CPlayerInfoGta* player);
    ...
    bool Update();
    void ProcessGunShot(VECTOR* origin, VECTOR* target);
