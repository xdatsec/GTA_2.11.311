...
{
    if (m_pVehicle && GamePool_Vehicle_GetAt(m_dwGTAId) && m_pVehicle->entity.vtable != (g_libGTASA + 0x808818))
    {
        if (bInv)
        {
    ...
    if(m_pVehicle)
    {
        if(GetVehicleSubtype() == VEHICLE_SUBTYPE_CAR)
        {
            if(!dwPanelDamage && !dwDoorDamage && !byteLightDamage)
            {
                ...
                {
                    // CAutoMobile::Fix
                    (( void (*)(VEHICLE_TYPE*))(g_libGTASA+0x69F44C))(m_pVehicle);
                    return;
                }
            }
            ...
            (( void (*)(VEHICLE_TYPE*))(g_libGTASA+0x69F94C))(m_pVehicle);
        }
    }
}
