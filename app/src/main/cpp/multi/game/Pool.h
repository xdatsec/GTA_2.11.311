            ...
            if (++m_nFirstFree >= m_nSize) {
                if (bReachedTop) {
                    ...
                    if constexpr (DontDebugCheckAlloc) {
                        LOGI("Allocataion failed!"); // Code can handle alloc failures
                    } else {
                        LOGI("Allocataion failed!"); // Code can handle alloc failures // Code can't handle alloc failures, so break
                    }
                    return nullptr;
                }
                bReachedTop = true;
