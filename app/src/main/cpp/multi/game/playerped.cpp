...
CPlayerPed::~CPlayerPed()
{
    ...
    if (m_pPed && (GamePool_Ped_GetAt(m_dwGTAId) != 0) && m_pPed->entity.vtable != (g_libGTASA + 0x808818) /* CPlaceable */)
    {
...
bool CPlayerPed::IsInVehicle()
{
    ...
    if((*(uint8_t *)((uintptr_t)m_pPed + 1412) & 1) == 1) {
        return true;
    }
...
bool CPlayerPed::IsAPassenger()
{
    ...
    if (pVehicle1 && *(uint8_t *)((uintptr_t)m_pPed + 1412) << 31 != 0)
    {
...
VEHICLE_TYPE* CPlayerPed::GetGtaVehicle()
{
    ...
    uintptr_t pVehicle = *(uintptr_t *)((uintptr_t)m_pPed + 1800);
...
bool IsTaskRunNamedOrSlideToCoord(void* pTask)
{
    ...
    if (dwVTable == (g_libGTASA + 0x8180B8) || dwVTable == (g_libGTASA + 0x810B40)) // CTaskSimpleSlideToCoord CTaskSimpleRunNamedAnim
    {
        return true;
    }
    return false;
...
uint32_t CPlayerPed::GetCurrentAnimationIndex()
{
    ...
    PED_TYPE * m_pPed2 = (PED_TYPE *)m_pPed;
    if (!m_pPed2->entity.m_pRwObject)
    {
        return 0;
    }
    sizeof(PED_TYPE);
    ...
    if (pIntelligence)
    {
        void* pTask = pIntelligence->Task.m_aPrimaryTasks[3];
        ...
        if (pTask)
        {
            ...
            const char* szName = (const char*)((uintptr_t)pTask + 25);
            const char* szGroupName = (const char*)((uintptr_t)pTask + 49);
...
bool CPlayerPed::IsPlayingAnimation(int iIndex)
{
    ...
    uintptr_t blendAssoc = ((uintptr_t(*)(uintptr_t* clump, const char* szName))(g_libGTASA + 0x362544))
            (reinterpret_cast<uintptr_t *>(m_pPed->entity.m_pRwObject), pNameAnim);	// RpAnimBlendClumpGetAssociation
...
{
    uintptr_t* pBlendAssocGroup = *(uintptr_t * *)(g_libGTASA + 0x8B8538); // CAnimManager::ms_aAnimAssocGroups
...
void CPlayerPed::RemoveWeaponWhenEnteringVehicle()
{
    if (m_pPed) {
        ...
        ((void(*)(PED_TYPE*, int))(g_libGTASA + 0x56F368))(m_pPed, 0);
    }
}
// 0.3.7
void CPlayerPed::SetInitialState()
{
    ...
    ((void(*)(PED_TYPE*))(g_libGTASA + /*0x458D1C*/0x597E58))(m_pPed);
}
// 0.3.7
...
void CPlayerPed::SetModelIndex(uint uiModel)
{
    ...
    if (m_pPed)
    {
        ...
        CHook::RET(g_libGTASA + 0x521EF0);
        DestroyFollowPedTask();
        CEntity::SetModelIndex(uiModel);
        // CAEPedSpeechAudioEntity::Initialise
        ((void (*)(uintptr_t, uintptr_t))(g_libGTASA + 0x3710B8))(((uintptr_t)m_pPed + 1336), (uintptr_t)m_pPed);
    }
}
...
void CPlayerPed::ClearWeapons()
{
    ...
    *(uint8_t*)(g_libGTASA + 0xA01210) = m_bytePlayerNumber; // CWorld::PlayerInFocus
    ((void (*)(PED_TYPE*))(g_libGTASA + 0x568A28))(m_pPed); // CPed::ClearWeapons
    *(uint8_t*)(g_libGTASA + 0xA01210) = 0;
}
...
void CPlayerPed::GiveWeapon(int iWeaponId, int iAmmo)
{
    if (m_pPed && GamePool_Ped_GetAt(m_dwGTAId))
    {
        ...
        if (iWeaponModelID != -1)
        {
            ...
            *(uint8_t*)(g_libGTASA + 0x568A28) = m_bytePlayerNumber; // CWorld::PlayerInFocus
            // CPed::GiveWeapon
            ((void (*)(PED_TYPE*, int, int, bool))(g_libGTASA + 0x568610))(m_pPed, iWeaponId, iAmmo, true);
            // sub_1009C4B0
            SetArmedWeapon(iWeaponId, 0);
            *(uint8_t*)(g_libGTASA + 0x568A28) = 0; // CWorld::PlayerInFocus
        }
    }
}
    ...
    if (m_pPed && GamePool_Ped_GetAt(m_dwGTAId))
    {
        *pbyteCurrentPlayer = m_bytePlayerNumber;
        ...
        if (unk)
        {
            ...
            ((void (*)(PED_TYPE*, int))(g_libGTASA + 0x56F148))(m_pPed, weapon);
            // sub_1009C4B0
        }
        else
        {
...
{
    if (!m_pPed) return;
    if (m_pEntity->vtable == g_libGTASA + 0x808818) return; // CPlaceable

    // CPed::GetBonePosition
    ((void (*)(PED_TYPE*, VECTOR*, int, int))(g_libGTASA + 0x56E738))(m_pPed, vecOut, iBoneID, 0);
}
// 0.3.7
void CPlayerPed::GetTransformedBonePosition(int iBoneID, VECTOR* vecOut)
{
    ...
    ((void (*)(PED_TYPE*, VECTOR*, int, int))(g_libGTASA + 0x56BC3C))(m_pPed, vecOut, iBoneID, 0);
}
...
{
    if (m_pPed) {
        return *(float*)(m_pPed->dwPlayerInfoOffset + 0x68);
    }
    else {
        return 0.0f;
    }
...
void CPlayerPed::SetAimZ(float fAimZ)
{
    ...
    {
        if (m_pPed) {
            *(float*)(m_pPed->dwPlayerInfoOffset + 0x68) = fAimZ;
        }
    }
}
// 0.3.7
...
void CPlayerPed::PutDirectlyInVehicle(uint32_t dwVehicleGTAId, uint8_t byteSeatID)
{
    ...
    if (pGtaVehicle->fHealth != 0.0f && pGtaVehicle->entity.vtable != (g_libGTASA + 0x808818))
    {
...
int CPlayerPed::GetCurrentVehicleID()
{
    ...
    VEHICLE_TYPE *pVehicle = *(VEHICLE_TYPE**)((uintptr_t)m_pPed + 0x708);
...
int CPlayerPed::GetVehicleSeatID()
{
    ...
    if( GetActionTrigger() == ACTION_INCAR && (pVehicle = *(VEHICLE_TYPE**)((uintptr_t)m_pPed + 0x708)) != 0 )
    {
...
void CPlayerPed::SetAttachedObject(int index, NEW_ATTACHED_OBJECT* pNewAttachedObject)
{
    ...
    if (m_pPed && m_pPed->entity.vtable != (g_libGTASA + 0x808818))
    {
        if (m_pPed->entity.m_pRwObject)
        {
            if (index >= 0 && index < 10)
        ...
        if (m_bObjectSlotUsed[i])
        {
            if (m_pAttachedObjects[i] && m_pAttachedObjects[i]->m_pEntity)
            {
                if (IsAdded())
                {
                    ...
                    // CPhysical::Remove
                    ((void (*) (ENTITY_TYPE*))(*(uintptr_t*)(m_pAttachedObjects[i]->m_pEntity->vtable + 0x20)))(m_pAttachedObjects[i]->m_pEntity);
...
void CPlayerPed::GetBoneMatrix(MATRIX4X4* matOut, int iBoneID)
{
    if (m_pPed && m_pPed->entity.vtable != (g_libGTASA + 0x808818))
    {
        if (m_pPed->entity.m_pRwObject)
        {
            ...
            uintptr_t pAnimHierarchy = ((uintptr_t(*)(uintptr_t))(g_libGTASA + 0x5E1F34))(m_pPed->entity.m_pRwObject);

            // RpHAnimIDGetIndex
            int index = (( int (*)(uintptr_t, int))(g_libGTASA + 0x76EF98))(pAnimHierarchy, iBoneID) << 6;
...
void CPlayerPed::ClumpUpdateAnimations(float step, int flag)
{
    ...
    if (m_pPed)
    {
        ...
        if (pRwObj) {
            ...
            ((void (*)(uintptr_t, float, int))(g_libGTASA + 0x35C050))(pRwObj, step, flag);
        }
    }
}
...
void CPlayerPed::FireInstant()
{
    ...
    if (GetCurrentWeapon() == WEAPON_SNIPER)
    {
        ...
        {
            // CWeapon::FireSniper
            ((void (*)(WEAPON_SLOT_TYPE*, PED_TYPE*, uint32_t, uint32_t))(g_libGTASA + 0x6EA8E4))(
    ...
    else
    {
        ...
        ((void (*)(WEAPON_SLOT_TYPE*, PED_TYPE*, VECTOR*, VECTOR*, uint32_t, uint32_t, uint32_t, uint32_t, uint32_t))(g_libGTASA + 0x6E921C))(
...
VECTOR* CPlayerPed::GetCurrentWeaponFireOffset()
{
    ...
    uintptr_t pWeaponInfo = ((uintptr_t(*)(int, int))(g_libGTASA + 0x6F21A8))(pSlot->dwType, 1);
...
void CPlayerPed::ApplyCrouch()
{
    ...
    if (!(m_pPed->dwStateFlags & 256))
    {
        if (!IsCrouching())
        {
            ...
            {
                ((int (*)(CPedIntelligence*, uint16_t))(g_libGTASA + 0x593B38))(m_pPed->pPedIntelligence, 0);
            }
        }
    }
}
...
bool CPlayerPed::IsInJetpackMode()
{
    if(m_pPed && IsAdded() && !IsInVehicle()  &&
            m_pPed->pPedIntelligence->Task.pdwJumpJetPack)
    {
        return GetTaskTypeFromTask( m_pPed->pPedIntelligence->Task.pdwJumpJetPack) == 1303;
    }
...
void CPlayerPed::StopJetpack()
{
    ...
    if(IsInJetpackMode())
    {
        uintptr_t * dwJetPackTask = m_pPed->pPedIntelligence->Task.pdwJumpJetPack;

        // CTaskSimpleJetPack::~CTaskSimpleJetPack
        (( void (*)(uintptr_t * ))(g_libGTASA+0x66C28C))(dwJetPackTask); // CTaskSimpleJetPack::~CTaskSimpleJetPack

        m_pPed->pPedIntelligence->Task.pdwJumpJetPack = 0;
    }
}
...
int CPlayerPed::HasHandsUp()
{
    ...
    if(m_pPed->pPedIntelligence->Task.pdwJumpJetPack == NULL) return false;
    uint32_t dwJmpVtbl = m_pPed->pPedIntelligence->Task.pdwJumpJetPack[0];
    if(dwJmpVtbl == g_libGTASA+0x807B98) return true;
    return false;
}
...
bool CPlayerPed::IsPerformingCustomAnim()
{
    ...
    if(m_pPed->pPedIntelligence->Task.pdwJumpJetPack) return true;
    return false;
}
...
{
    if(m_pPed && !IsInVehicle() && m_pPed->pPedIntelligence && m_pPed->pPedIntelligence->Task.pdwJumpJetPack)
        return GetTaskTypeFromTask(m_pPed->pPedIntelligence->Task.pdwJumpJetPack) == 211;
...
{
    return (VEHICLE_TYPE*)(m_pPed + 1752);
}

ENTITY_TYPE* CPlayerPed::GetGtaContactEntity()
{
    return (ENTITY_TYPE*)(m_pPed + 1784);
}

bool CPlayerPed::IsTakeDamageFallTask()
{
    if(m_pPed && !IsInVehicle() && m_pPed->pPedIntelligence && m_pPed->pPedIntelligence->Task.pdwDamage)
        return GetTaskTypeFromTask(m_pPed->pPedIntelligence->Task.pdwDamage) == 208;
...
{
    if(m_pPed && m_pPed->pPedIntelligence && m_pPed->pPedIntelligence->Task.pdwJumpJetPack)
    {
        int iType = GetTaskTypeFromTask(m_pPed->pPedIntelligence->Task.pdwJumpJetPack);
...
{
    if(m_pPed && m_pPed->pPedIntelligence && m_pPed->pPedIntelligence->Task.pdwJumpJetPack)
        return GetTaskTypeFromTask(m_pPed->pPedIntelligence->Task.pdwJumpJetPack) == 704;
...
bool CPlayerPed::IsSitTask()
{
    if(m_pPed && m_pPed->pPedIntelligence && m_pPed->pPedIntelligence->Task.pdwJumpJetPack)
    {
        return (GetTaskTypeFromTask(m_pPed->pPedIntelligence->Task.pdwJumpJetPack) == 221 ||
                GetTaskTypeFromTask(m_pPed->pPedIntelligence->Task.pdwJumpJetPack) == 220);
    }
...
{
    if(m_pPed->entity.vtable == g_libGTASA+0x8080C8) return;
...
{
    ((int(*)(uintptr_t, uint8_t))(g_libGTASA + 0x56F148))((uintptr_t)m_pPed, weaponType);
}
    ...
    for (size_t i = 0; i < MAX_VEHICLES; i++) {
        if (pVehiclePool->GetSlotState(i)) {
            ...
            if (pVehicle && pVehicle->IsAdded()) {
                if (pVehicle->m_pVehicle == *(VEHICLE_TYPE**)((uintptr_t)m_pPed + 0x708)) {
                    return pVehicle;
                }
            }
        }
...
{
    if(!m_pPed || !GamePool_Ped_GetAt(m_dwGTAId) || !IsInVehicle() ||
       !m_pPed->pPedIntelligence || !m_pPed->pPedIntelligence->Task.pdwJumpJetPack)
    {
        return false;
    }

    return GetTaskTypeFromTask(m_pPed->pPedIntelligence->Task.pdwJumpJetPack) == 1022;
}
...
void CPlayerPed::StopPassengerDriveByMode()
{
    ...
    if(IsInPassengerDriveByMode())
    {
        ...
        (( void (*)(uintptr_t *))(g_libGTASA+0x601A28))(dwJetPackTask);
