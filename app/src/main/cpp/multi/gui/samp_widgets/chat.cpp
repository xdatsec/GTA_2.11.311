...
extern Widget* callerid;

ImVec2 m_chatSize = ImVec2(299.0f, 0.0f);
Chat* pChat = nullptr;
Widget* callerid = nullptr;
Chat::Chat()
...
void Chat::addMessage(const std::string& message, const ImColor& color)
{
	...
	int red   = (int)(color.Value.x * 255.0f);
	int green = (int)(color.Value.y * 255.0f);
	int blue  = (int)(color.Value.z * 255.0f);

	char buf[1024];
	snprintf(buf, sizeof(buf), "{%02X%02X%02X}%s", red, green, blue,message.c_str());


	callerid = this;

    float maxWidth = m_chatSize.x - 10.0f;
...
void Chat::addChatMessage(const std::string& message, const std::string& nick, const ImColor& nick_color)
{
	...
	int rednick_color   = (int)(nick_color.Value.x * 255.0f);
	int greennick_color = (int)(nick_color.Value.y * 255.0f);
	int bluenick_color  = (int)(nick_color.Value.z * 255.0f);



	char nicknamefinal[1024];
	snprintf(nicknamefinal, sizeof(nicknamefinal), "{%02X%02X%02X}%s", rednick_color, greennick_color, bluenick_color, nick.c_str());


	char msgfinal[1024];
	snprintf(msgfinal, sizeof(msgfinal), "%s%s",nicknamefinal, message.c_str());

	callerid = this;
	// if (!m_showingChat) return;

    float nickWidth = nick.size() * 7.0f;
    float maxWidth = m_chatSize.x - 10.0f - nickWidth;
    float charWidth = 7.0f;
