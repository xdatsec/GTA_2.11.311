package com.rockstargames.oswrapper.ui.dialog;
    ...
    public void updateSizes(TextView textView) {
        ...
        for (int i = 0; i < this.mRowsList.size(); i++) {
            ...
            if (!this.mTablist) {
                ...
                for (int i2 = 0; i2 < 4; i2++) {
                    float textWidth2 = ((float) Util.getTextWidth(
                            Util.getColoredString(split[i2]).toString(),
                            textView.getPaint()
                    )) + Util.scale(this.mContext, 32.0f);

                    if (((float) this.mTabSizes[i2]) < textWidth2) {
                        this.mTabSizes[i2] = (int) textWidth2;
                    }
                }
            }
        }
